...
public class A02FrontEnd extends JFrame implements ItemListener, ActionListener{
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	try {
    		//update date ranges if selected
    		if(dateRange.isSelected()) {
    		from = fromDate.getText();
    		to = toDate.getText();
    		}
    		
    		if(allDates.isSelected()) {	selectedBox = "Events" ;}
    		if(dateRange.isSelected()) { selectedBox = "Period";}
	    	if(eventConference.isSelected()) { selectedBox += ",EventConference";}
	    	if ( eventJournal.isSelected()) { selectedBox += ",EventJournal";}
	    	if ( eventBook.isSelected()) { selectedBox += ",EventBook";}
	    	
    		//call to get response method to output results
  			queryOutput.setText(A02MiddleTier.getResponse(selectedBox, from, to));
